...
const AdvertisedProduct = () => {
  ...
  return <div>
    ...
    <h3 className="text-3xl my-8 text-slate-200 text-center">Best value for money products</h3>
    <div className="my-12 grid grid-cols-1 bg-base-200 p-8 gap-8 lg:grid-cols-3">
      {
          adsStatus.map(product => <div 
            key={product._id} 
          ...
          <div className="card-body h-full bg-slate-300 mx-3 mb-3">
            ...
            <div className="card-actions justify-center">
              <Link className="btn bg-slate-900 hover:bg-slate-700 hover:text-slate-200" to={`products/${product.category_id}`}>
                Visit
              </Link>
            </div>
          </div>
