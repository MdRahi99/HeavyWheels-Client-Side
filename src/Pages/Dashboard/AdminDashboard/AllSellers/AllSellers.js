...
const AllSellers = () => {
  ...
  const {
    ...
    queryFn: async () => {
      try {
        const res = await fetch("https://heavy-wheels-server.vercel.app/users/seller", {
          headers: {
  ...
  const handleVerifySeller = (id) => {
    fetch(`https://heavy-wheels-server.vercel.app/users/admin/${id}`, {
      method: "PUT",
      headers: {
        authorization: `bearer ${localStorage.getItem("accessToken")}`,
      },
  ...
  return (
    <div className="bg-slate-900 p-4 my-12 mx-4">
      <h2 className="text-3xl my-8 text-slate-200 text-center">All Sellers</h2>
